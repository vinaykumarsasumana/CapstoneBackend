...
namespace CoreWebApiJWT.Services
{
    ...
    public class IdentityService : IIdentityService
    {
        ...
        public IdentityService(DemoTokenContexts context,
        ...
        public async Task<ResponseModel<TokenModel>> LoginAsync(LoginModel login)
        {
            ...
            try
            {
                ...
                SellerRegistration loginUser = _context.SellerRegistrations.FirstOrDefault(c => c.EmailId == login.EmailId && c.SellerPassword == login.SellerPassword);
        ...
        public async Task<AuthenticationResult> AuthenticateAsync(SellerRegistration user)
        {
            ...
            try
            {
                ...
                ClaimsIdentity Subject = new ClaimsIdentity(new Claim[]
                    {
                    new Claim("SellerRegId", user.SellerRegId.ToString()),
                    new Claim("FirstName", user.FirstName),
                    new Claim("LastName",user.LastName),
                    new Claim("LastName",user.LastName),
                    new Claim("LastName",user.LastName),
                    new Claim("LastName",user.LastName),
                    new Claim("EmailId",user.EmailId==null?"":user.EmailId),
                    //new Claim("UserName",user.UserName==null?"":user.UserName),
                    new Claim(JwtRegisteredClaimNames.Jti, Guid.NewGuid().ToString()),
                    });
                //foreach (var item in GetUserRole(user.SellerRegId))
                //{
